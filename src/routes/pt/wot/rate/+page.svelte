<script lang="ts">
	...
	async function handleSubmit() {
		if (ratingDescription.length >= 1000)
			return alert('2000 caracteres é o máximo para a descrição da avaliação');

		if (score == undefined || businessAlreadyDone == undefined)
			return alert('Você esqueceu de preencher algum campo');
		...
		if (!nostrAuth) return alert('Chave secreta inválida');
		...
		if (!otherPubKey) return alert('Chave pública inválida');
		...
		if (confirm('Evento publicado. Deseja ver as avaliações?')) {
			goto('/pt/wot/table');
		}
...
>
	<div class="flex flex-col items-center gap-4 text-center">
		...
		<p class="max-w-2xl text-sm text-[rgb(var(--text-muted))] md:text-base">
			Publique uma avaliação de confiança para alguém com quem você transacionou no Nostr.
		</p>
	</div>
	...
	<Card class="mx-auto flex w-full max-w-4xl flex-col gap-6">
		{#if $nostrAuth?.pubkey}
			<div class="flex flex-col gap-2">
				<Label class="pb-1 font-semibold text-[rgb(var(--text))]">Seu npub:</Label>
				<pre
		...
		<div class="flex flex-col gap-2">
			<Label class="pb-1 font-semibold text-[rgb(var(--text))]"
				>Chave pública da outra pessoa:</Label
			>
			{#if npub}
				<Input
		...
		<div class="grid gap-5 lg:grid-cols-2">
			<Card padding="sm" variant="outline" class="flex flex-col gap-3">
				<Label class="pb-1 font-semibold text-[rgb(var(--text))]">
					Que nota você dá para esta pessoa?
				</Label>
			...
			<Card padding="sm" variant="outline" class="flex flex-col gap-3">
				<Label class="pb-1 font-semibold text-[rgb(var(--text))]">
					Você já fez negócios com esta pessoa?
				</Label>
				<div class="flex gap-4">
					<Label class="flex items-center gap-2 text-[rgb(var(--text))]">
		...
		<div class="flex flex-col gap-2">
			<Label class="pb-1 font-semibold text-[rgb(var(--text))]">Descrição da avaliação</Label>
			<Textarea
		...
		<div class="flex justify-end">
			<Button type="submit" variant="primary" size="lg" class="min-w-56">Criar avaliação</Button>
		</div>
	</Card>
</form>
